...
def fed_avg(cluster_id, client_ids, global_model):
    total_clients = len(client_ids)
    aggregated_coef = None
    aggregated_intercept = None

    for client_id in client_ids:
        with open(f'Cluster{cluster_id}/model_{client_id}.pkl', 'rb') as f:
...
def fed_prox(cluster_id, client_ids, global_model, mu=0.01):
    total_clients = len(client_ids)
    aggregated_coef = None
    aggregated_intercept = None
...
def fed_curv(cluster_id, client_ids, global_model, alpha=0.01):
    total_clients = len(client_ids)
    aggregated_coef = None
    aggregated_intercept = None
    prev_coef = None
